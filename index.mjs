...
subscriber.on("spin", (inputwild) => {
  let clientsArray = [];
  let sentarray = [];
  let clientsList = subscriber.sockets.adapter.rooms;
  ...
  if (
    subscriber.socket.id == choosenRandomUser &&
  ...
  sentarray.push(subscriber.socket.id);
  // io.to(sentarray).emit("spin", inputwild);
  connections.forEach((c) => c.send("spin", inputwild));
});

subscriber.on("wild", (inputwild, inputwildnum) => {
  let clientsArray = [];
  let clientsList = subscriber.sockets.adapter.rooms;
  let sentarray = [];
  for (let item of clientsList.keys()) clientsArray.push(item);
  ...
  sentarray.push(subscriber.socket.id);
  connections.forEach((c) => c.send("wild", inputwild));
});
